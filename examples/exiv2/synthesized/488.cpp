...
extern "C" int LLVMFuzzerTestOneInput_488(const uint8_t *data, size_t size) {
    ...
    // Test c_data()
    try {
        size_t offset1 = size > 1 ? data[0] % size : 0;
        const Exiv2::byte* c_data_ptr = buf.c_data(offset1);
    ...
    // Test data()
    try {
        size_t offset2 = size > 1 ? data[0] % size : 0;
        Exiv2::byte* data_ptr = buf.data(offset2);
    ...
    // Test c_str()
    try {
        size_t offset3 = size > 1 ? data[0] % size : 0;
        const char* c_str_ptr = buf.c_str(offset3);
