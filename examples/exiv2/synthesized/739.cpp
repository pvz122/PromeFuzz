...
extern "C" int LLVMFuzzerTestOneInput_739(const uint8_t *Data, size_t Size) {
    ...
    try {
        ...
        // Create a dummy XmpKey using public constructor
        Exiv2::XmpKey key("Xmp.dc.title");

        // Test findKey()
        auto foundIt = xmpData.findKey(key);

        // Test add() with dummy Xmpdatum
        Exiv2::Xmpdatum xmpDatum(key);
        xmpData.add(xmpDatum);

        // Test iterators again after modification
        beginIt = xmpData.begin();
        endIt = xmpData.end();

        // Test xmpPacket() again
        const std::string& packet2 = xmpData.xmpPacket();
