...
extern "C" int LLVMFuzzerTestOneInput_520(const uint8_t *Data, size_t Size) {
    ...
    try {
        // Initialize ValueType object with a concrete type (e.g., uint32_t)
        ...
        if (Size > 0) {
            ...
            size_t copied = valueType.copy(buf, Exiv2::littleEndian);
            delete[] buf;
        }

        if (Size > 0) {
            Exiv2::byte* buf = new Exiv2::byte[Size];
            size_t copied = valueType.copy(buf, Exiv2::bigEndian);
            delete[] buf;
        }
