...
extern "C" int LLVMFuzzerTestOneInput_714(const uint8_t *Data, size_t Size) {
    ...
    try {
        (void)storage.unsafeAt(Size % vec.size());
    } catch (...) {}

    try {
        (void)storage.unsafeGetIteratorAt(Size % vec.size());
    } catch (...) {}
