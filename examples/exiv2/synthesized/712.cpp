...
extern "C" int LLVMFuzzerTestOneInput_712(const uint8_t *Data, size_t Size) {
    ...
    if (Size > 4) {
        valueType.setDataArea(Data, Size % 16); // Limit size to avoid excessive memory usage
    }
    valueType.read(Data, Size % 16, byteOrder); // Limit size to avoid excessive memory usage
    ...
    dataValue.read(Data, Size % 16, byteOrder); // Limit size to avoid excessive memory usage
