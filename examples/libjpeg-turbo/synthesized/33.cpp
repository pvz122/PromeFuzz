...
extern "C" int LLVMFuzzerTestOneInput_33(const uint8_t *Data, size_t Size) {
    ...
    int width = 16;
    int height = 16;
    int pitch = width * 3; // Adjusted for 3 components per pixel
    ...
    tjSaveImage(filename, buffer, width, width * 3, height, pixelFormat, flags);

    unsigned short *buffer16 = (unsigned short *)malloc(width * height * 3 * sizeof(unsigned short));
    if (!buffer16) {
        free(buffer);
        tjDestroy(handle);
        return 0;
    ...
    tjDecompress(handle, jpegBuf, Size, dstBuf, width, width * 3, height, 3, flags);
    ...
    int result = tj3Decompress16(handle, jpegBuf, Size, buffer16, pitch, pixelFormat);
