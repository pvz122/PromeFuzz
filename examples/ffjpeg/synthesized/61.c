...
int LLVMFuzzerTestOneInput_61(const uint8_t *Data, size_t Size) {
    if (Size < 192) return 0; // Ensure enough data for initialization (64 + 64 + 64)

    int ftab[64];
    int qtab[64];
    int du[64];

    // Initialize ftab and qtab with fuzz data
    for (int i = 0; i < 64; i++) {
        ftab[i] = (int)Data[i];
        qtab[i] = (int)Data[i + 64];
    }

    // Initialize du with fuzz data
    for (int i = 0; i < 64; i++) {
        du[i] = (int)Data[i + 128];
    }
    ...
    // Perform quantization on the block
    for (int i = 0; i < 64; i++) {
        if (qtab[i] == 0) qtab[i] = 1; // Ensure no division by zero
    }
    quant_encode(du, qtab);

    // Perform 2D IDCT on the block
    idct2d8x8(du, ftab);
