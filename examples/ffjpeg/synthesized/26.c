...
int LLVMFuzzerTestOneInput_26(const uint8_t *Data, size_t Size) {
    if (Size < 128) return 0; // Ensure we have enough data

    int du[64];
    int ftab[64];
    int qtab[64];

    // Initialize du, ftab, and qtab with random values from the input data
    for (int i = 0; i < 64; i++) {
        du[i] = (i < Size) ? (int)Data[i] : 0;
        qtab[i] = (i + 1 < Size) ? (int)Data[i + 64] : 1; // Avoid zero division
    }

    // Ensure qtab does not contain zero values to prevent division by zero
    for (int i = 0; i < 64; i++) {
        if (qtab[i] == 0) {
            qtab[i] = 1; // Set to 1 to avoid division by zero
        }
    }
