...
// loguru::format_value at loguru.hpp:744:32 in loguru.hpp
#include <iostream>
#include <sstream>
#include <string>
#include <vector>
...
extern "C" int LLVMFuzzerTestOneInput_87(const uint8_t *Data, size_t Size) {
    ...
    // Initialize loguru
    int argc = 1;
    char* argv[] = {const_cast<char*>("fuzz_driver"), nullptr}; // Ensure argv is null-terminated
    loguru::init(argc, argv);

    // Test loguru::format_value
    long value = static_cast<long>(Data[0]);
    ...
    loguru::log(loguru::Verbosity_INFO, "./dummy_file", 42, format_str.c_str(), static_cast<int>(data_str.size()), data_str.c_str());

    // Test loguru::ec_to_text
    std::string data_str2(reinterpret_cast<const char*>(Data), Size);
    loguru::Text text4 = loguru::ec_to_text(data_str2.c_str());

    // Test loguru::raw_log
    loguru::raw_log(loguru::Verbosity_INFO, "./dummy_file", 42, format_str.c_str(), static_cast<int>(data_str.size()), data_str.c_str());
