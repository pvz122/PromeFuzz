...
// loguru::ec_to_text at loguru.cpp:1869:2 in loguru.hpp
...
extern "C" int LLVMFuzzerTestOneInput_101(const uint8_t *Data, size_t Size) {
    ...
    // Test loguru::ec_to_text with double
    double double_data = 0.0;
    if (Size >= sizeof(double)) {
        std::memcpy(&double_data, Data, sizeof(double));
    }
    loguru::Text text1 = loguru::ec_to_text(double_data);

    // Test loguru::ec_to_text with EcHandle
    ...
    // Test loguru::ec_to_text with string
    std::string data_str(reinterpret_cast<const char*>(Data), Size);
    loguru::Text text5 = loguru::ec_to_text(data_str.c_str());
