...
int LLVMFuzzerTestOneInput_14(const uint8_t *Data, size_t Size) {
    ...
    img.planes[1] = (unsigned char *)Data + img.w * img.h;
    img.planes[2] = (unsigned char *)Data + img.w * img.h + (img.w / 2) * (img.h / 2);

    vpx_codec_encode(&ctx, &img, 0, 1, 0, VPX_DL_REALTIME);

    // Fuzz vpx_codec_err_to_string
    const char *err_str = vpx_codec_err_to_string(ctx.err);
