...
extern "C" int LLVMFuzzerTestOneInput_40(const uint8_t *Data, size_t Size) {
    ...
    // Set options based on fuzz input
    options.set_posix_syntax(Data[0] & 0x01);
    options.set_never_capture(Data[0] & 0x02);
    options.set_literal(Data[0] & 0x04);
    options.set_perl_classes(Data[0] & 0x08);
