...
extern "C" int LLVMFuzzerTestOneInput_28(const uint8_t *Data, size_t Size) {
    ...
    aom_image_t *img_ptr = aom_img_alloc_with_border(nullptr, fmt, d_w, d_h, align, size_align, border);
    if (img_ptr) {
        aom_img_flip(img_ptr);
        aom_img_free(img_ptr);
    }

    img_ptr = aom_img_alloc(nullptr, fmt, d_w, d_h, align);
    if (img_ptr) {
        aom_img_flip(img_ptr);
        aom_img_free(img_ptr);
    }
    ...
    if (img_data) {
        img_ptr = aom_img_wrap(nullptr, fmt, d_w, d_h, align, img_data);
        if (img_ptr) {
            aom_img_flip(img_ptr);
            aom_img_free(img_ptr);
        }
