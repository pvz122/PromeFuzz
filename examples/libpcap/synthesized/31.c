...
int LLVMFuzzerTestOneInput_31(const uint8_t *Data, size_t Size) {
    ...
    int *tstamp_types;
    pcap_list_tstamp_types(pcap, &tstamp_types);
    if (tstamp_types) pcap_free_tstamp_types(tstamp_types);

    // Fuzz pcap_tstamp_type_val_to_name
    int tstamp_type_val = Data[0] % 10;
    pcap_tstamp_type_val_to_name(tstamp_type_val);

    // Fuzz pcap_tstamp_type_name_to_val
    char tstamp_type_name[10];
    snprintf(tstamp_type_name, sizeof(tstamp_type_name), "type%d", tstamp_type_val);
    pcap_tstamp_type_name_to_val(tstamp_type_name);

    // Fuzz pcap_set_tstamp_type
    pcap_set_tstamp_type(pcap, tstamp_type_val);

    // Fuzz pcap_tstamp_type_val_to_description
    pcap_tstamp_type_val_to_description(tstamp_type_val);
